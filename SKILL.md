...
### continuous-messaging 依赖缺失（GitHub Issue #2）
SKILL.md 和 manifest.json 强制 `requires: [continuous-messaging]`，但该skill是 Hermes Agent 本地skill（`~/.hermes/skills/productivity/continuous-messaging/`），从未发布到GitHub。**非 Hermes 用户 clone 此仓库后会卡住**。

**解决方案**（待执行）：
1. 把 continuous-messaging 核心规则（2-4条短消息、每条≤3行、♪收尾）内联到 SKILL.md 作为兜底
2. 同时发布 duhca/continuous-messaging 独立仓库供 Hermes 用户加载

### GitHub 版本滞后
GitHub 仓库停留在 V7.0，本地已是 V8.0。V8.0 新增：红线#4（不自称头衔）、红线#5（身世修正）、导航工具、MiMo验证限制。需推送更新。
